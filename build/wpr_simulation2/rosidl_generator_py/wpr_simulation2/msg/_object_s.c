...
bool wpr_simulation2__msg__object__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("wpr_simulation2.msg._object.Object", full_classname_dest, 34) == 0);
  }
  wpr_simulation2__msg__Object * ros_message = _ros_message;
  {  // name
  ...
  {  // probability
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      double * dest = ros_message->probability.data;
      rc = PyBuffer_ToContiguous(dest, &view, view.len, 'C');
      if (rc < 0) {
        PyBuffer_Release(&view);
        Py_DECREF(field);
      ...
      PyObject * seq_field = PySequence_Fast(field, "expected a sequence in 'probability'");
      if (!seq_field) {
        Py_DECREF(field);
        return false;
      }
...
PyObject * wpr_simulation2__msg__object__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("wpr_simulation2.msg._object");
  ...
  {  // probability
    PyObject * field = NULL;
    field = PyObject_GetAttrString(_pymessage, "probability");
    if (!field) {
      return NULL;
    }
    assert(field->ob_type != NULL);
    ...
    if (ros_message->probability.size > 0) {
      // populating the array.array using the frombytes method
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      double * src = &(ros_message->probability.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->probability.size * sizeof(double));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
